...
def create_skeleton(
    ...
    for bone in bones:
        ...
        rotmat = bone.rotate_matrix_as # type: Matrix
...
def create_mesh(
    ...
    for submesh in submeshes:
        ...
        me.polygons.foreach_set('loop_total', [3] * submesh.face_count)
        ...
        if submesh.geometry.has_texture_coord:
            ...
            for texcorrd_index in range(nd_texcoords.shape[0]):
                ...
                uv_layer.name='UVLayer{}'.format(texcorrd_index)
                uv_layer.data.foreach_set('uv', nd_texcoords[texcorrd_index])

        if submesh.geometry.has_colors:
            nd_colors, nd_alphas = submesh.get_colors(is_rgba=False)
            color_data = me.vertex_colors.new(name='Colour{}'.format(submesh_index))
            color_data.data.foreach_set('color', nd_colors)
            alpha_data = me.vertex_colors.new(name='Alpha{}'.format(submesh_index))
            alpha_data.data.foreach_set('color', nd_alphas)
        ...
        if cleanup_vertices == 'KEEP_FACE':
            ...
            face_dict = {} # type: Dict[Vector, int]
            dupulicate_faces = [] # type: List[int]
